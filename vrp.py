...
def solve_cvrp_clarke_wright(n, Q, D_full, q):
    ...
    for i in range(len(routes)):
        routes[i] = two_opt(routes[i], D_full)
...
def two_opt(route, D):
    """Optimize a single route using 2-Opt."""
    best_route = route[:]
    improved = True
    while improved:
        improved = False
        for i in range(1, len(best_route) - 2):  # Exclude depot at ends
            for j in range(i + 1, len(best_route) - 1):
                # Calculate delta distance for swap
                old_edges = (D[best_route[i-1]][best_route[i]] +
                            D[best_route[j]][best_route[j+1]])
                new_edges = (D[best_route[i-1]][best_route[j]] +
